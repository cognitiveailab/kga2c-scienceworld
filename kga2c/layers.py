...
class PackedEncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden=None):
        ...
        idx = (lengths-1).view(-1,1).expand(len(lengths), output.size(2)).unsqueeze(0).to(device)
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        input = input.unsqueeze(0).expand(100, -1, -1)
...
class DecoderRNN2(nn.Module):
    ...
    def forward(self, input, hidden, encoder_output, graph_mask=None):
        ...
        output = torch.cat((embeds, encoder_output), dim=-1)
